...
export default function ChatInterface({ selectedDoc }: Props) {
  ...
  const ctxString = () =>
    snips.map(s=>`[${s.filename} p.${s.page} ${s.start}-${s.end}] ${s.text}`).join('\n\n');
  ...
  const send = async () => {
    ...
    setMessages(m=>[...m,{id:crypto.randomUUID(),type:'user',content:input,timestamp:new Date()}]);
    ...
    try{
      ...
      setMessages(m=>[...m,{id:aiId,type:'ai',content:'',timestamp:new Date(),sources}]);

      [...answer].forEach((ch,i)=>
        setTimeout(()=> setMessages(m=>m.map(x=>x.id===aiId? {...x,content:answer.slice(0,i+1)}:x)), i*20)
      );
      setTimeout(()=>setTyping(false), answer.length*20+100);

    }catch{
      setMessages(m=>[...m,{id:crypto.randomUUID(),type:'ai',content:'⚠️ backend error',timestamp:new Date()}]);
      setTyping(false);
    }
  };

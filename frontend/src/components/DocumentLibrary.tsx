...
export default function DocumentLibrary({onSelectDoc,selectedDoc,noWorkspace}:Props){
  const {data:docs=[],isLoading,refetch}=useQuery({queryKey:['documents'],queryFn:fetchDocs});
  ...
  const groups = useMemo(()=>{
    ...
    return docs.reduce<Record<string,Doc[]>>((acc,d)=>{
      const w=d.workspace||'default'; (acc[w]??=[]).push(d); return acc;
    },{});
  },[docs,noWorkspace]);
          ...
          {list.map(doc=>(
            <Card key={doc._additional.id}
                  ...
                  className={`p-3 mb-2 cursor-pointer ${
                    selectedDoc===doc._additional.id?'bg-primary/10 border-primary':'hover:bg-card/70'
                  }`}>
              <div className="flex items-center gap-3">
                ...
                <Badge variant="secondary" className="text-xs">
                  {doc.processed ? <><Zap className="w-3 h-3 mr-1"/>Ready</>
